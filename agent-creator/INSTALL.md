...
> 本文件位于 agent-creator **开发工作区根**，说明如何把**成品** `skills/agent-creator/` 安装到各客户端的技能目录。成品是自包含完整体：安装 = 把该目录完整放置到目标客户端的技能目录，跑完下方验证关卡即完成，不依赖任何外部工具。
...
```bash
cd skills/agent-creator     # 成品目录（在 agent-creator 工作区根执行本手册时）
...
把**成品目录**（`<仓库根>/agent-creator/skills/agent-creator/`）整个复制到第 0 步选定的落点，目录名保持 `agent-creator`。复制时排除 `__pycache__/` 等缓存噪音。

```bash
# Unix / WSL 示例（claude 全局）：从 agent-creator 工作区根执行
cp -r skills/agent-creator ~/.claude/skills/agent-creator

# Windows PowerShell 示例（opencode 工作区，$root = 目标仓库根）：
Copy-Item -Recurse skills/agent-creator "$root\.opencode\skills\agent-creator"
```
...
本技能内部命令一律以**技能目录为根**书写（见 SKILL.md「资源路径基准」），脚本可自定位内部资源；客户端只把 SKILL.md 正文交给模型时如何定位技能目录，同样见 SKILL.md「资源路径基准」，此处不重复。
