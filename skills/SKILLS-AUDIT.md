...
| 经 skill-creator 生成流程 | 2 ✅ / 1 直接导入 ⚠️（需补 evolutions） |
...
- ⚠️ **prd-generator 审核中（规则 2、3、4）**：待审从 `snarktank/ralph` 的 prd skill 直接导入——frontmatter 已补齐本地 schema（category: product / risk: safe / source: community），含示例/限制章节，`validate_skills.py --strict` 通过。待补 full evolutions 记录。
...
> 上游对比/导入记录（位于技能创建器 `skill-creator/skills/skill-creator/evolutions/`）：`2026-09-02-compare-pr-summarizer.md`（自建版采纳上游精华 + 差异化定位）；`2026-09-03-import-code-review-skill.md`（本地无候选 A → 直接导入上游 B）。
...
| prd-generator | `skills/product-design/prd-generator/SKILL.md` | product | safe | community | 上游 `snarktank/ralph` 的 prd skill（交互式需求规格化） | ⚠️ | ⚠️ 审核中（待补 evolutions 记录） |
