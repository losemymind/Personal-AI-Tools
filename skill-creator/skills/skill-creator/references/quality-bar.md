...
基于 agentic-awesome-skills 的 `quality-bar.md` 适配。技能必须达到以下 **8 项质量检查** 才能视为合格。其中部分由 `scripts/validate_skills.py` 自动执行（标注「自动」），其余需要人工评审。
...
### 7. 写作规律（人工；读取见 `SKILL.md`「读取规则」中写作规律文档）
